...
void printHelp(void) {
  fprintf(stderr, "%s [-d] [-h] [-s source1,source2,..] [-t target]\n",
          programName);
  fprintf(stderr, "\n");
  fprintf(stderr, " -d                     : debug mode to demonstate each "
                  "step of the moves\n");
...
void runTest(char string[], char target[]) {
  ...
  // there is no guarantee that 1st str1 is passed to printf before it is
  // transformed by nested method transform
