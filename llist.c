...
int delListNodeInt(int val, struct ListNode **start, struct ListNode **end) {
  ...
  // we can potentially combine the logic inside the if ... then block with
  // logic after it, but it is also more obvious that we check if start matches
  // the value, and do a set of activities, else we search through the list for
  // a matching node and do another set of activities.
  ...
  tmp = *start;
  while (NULL != tmp->next && val != tmp->next->data.val)
    tmp = tmp->next;
...
struct ListNode *addListNodeInt(int val, struct ListNode **start,
  ...
  if (NULL == *start || val < (*start)->data.val) {
    node->next = *start;
    *start = node;
...
struct ListNode *enQueue(union ListNodeData data, struct ListNode **head) {
  ...
  if (NULL == *head) {
    *head = node;
    ...
    // this can be inefficient if the list is long, a proper way is that the
    // ListNode ADT data also point a pointer to the tail node, so that we can
    // insert without traversal.
...
int findListNodeRefIndex(struct ListNode *start, void *ref) {
  ...
  i = 0;
  while (NULL != start &&
         start->data.ref != ref) // to be generic, we need to pass in comparison
                                 // method for truly object identity
  {
    start = start->next;
    i++;
  }
...
int findListNodeIntIndex(struct ListNode *start, int val) {
  ...
  i = 0;
  while (NULL != start && start->data.val != val) {
    start = start->next;
    i++;
  }
  ...
  if (NULL == first) {
    ...
    while (NULL != first && NULL != second) {
      if (first->data.val == second->data.val) {
        ...
        while (NULL != first->next && first->next->data.val < second->data.val)
          first = first->next;
        ...
        while (NULL != second->next && second->next->data.val < first->data.val)
          second = second->next;
