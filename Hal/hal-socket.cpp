...
Hal_Socket::Hal_Socket(std::string_view ip4, int portno, bool writeOnly)
...
Hal_Socket::~Hal_Socket() {
  if (-1 != m_fd) {
    close(m_fd);
  }
}

std::optional<std::string> Hal_Socket::read() {
...
void Hal_Socket::write(std::string &item) {
