...
void arrangeDependencies(char packages[], char *dependencies[], int size,
  ...
  for (i = 0; (i < size) && (i < lastIndex); i++) {
    if (0x0 == packages[i]) {
      // do nothing
    } else {
      if (0 == strlen(dependencies[i])) {
      } else {
        char *p = dependencies[i];
        ...
        while ('\0' != *p) {
          ...
          for (j = i + 1; j < size; j++) {
            if (*p == packages[j]) {
              arrangeDependencies(&packages[j], &dependencies[j], size - j, 1);
              packages[j] = 0x0;
            }
          }
...
void runArrangeDependencies(void) {
  char packages1[] = {'a', 'b', 'c', 'd'};
  char *dependencies1[] = {"b", "c", "d", ""};

  arrangeDependencies(packages1, dependencies1, 4, 4);
  printf("\n");

  char packages2[] = {'a', 'b', 'c', 'd'};
  char *dependencies2[] = {"bc", "", "d", ""};

  arrangeDependencies(packages2, dependencies2, 4, 4);
  printf("\n");
}
