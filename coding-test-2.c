...
int determineIfStringCanBeSegmented(char *dictionary[], char *s) {
  ...
  for (i = 0; NULL != dictionary[i]; i++) {
    ...
    if (strncmp(dictionary[i], s, len) == 0) {
      ...
      // segemented, we can be stack overflow, another way is to maintain
      ...
      // previous poistion in string to explore into other search.
    }
  }
 ...
 * 3+     6+     9+
 *
 * new 3rd row is 9, 6 and 3, backard by 1 space, so we pick 9, 6 and 3 into 3rd
 * row, so the final matrix is:
 *
 * 7      4      1
 * 8      5      2
 * 9      6      3
