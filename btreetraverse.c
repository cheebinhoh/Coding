 ...
 *
 * If we are at a node, we will process left child node (if any), and then process right child node
 ...
 * - if root->left is not NULL, we need to process left branch and then backtrack to right branch
 *   and then parent node of the branches, so we store root (parent node) in rightPendingList
 *   (the name indicates that right branch is pending to be processed), and then assign root->left to be
 *   new root and continue the loop.
 * - if root->left is NULL, but root->right is not NULL, then we need to process the right node and then
 *   back to the root later, let store the root in topPendingList so we want to backtrack to it after we
 *   exhaust the right branch, we assign root->right to be new root and continue the loop.
 ...
 *      topPendingList that it right branch is not equal to root.
 *
 *   -- once we backtrack enough from right branch either by exhausting the topPendingList, or reach top
 *      of the topPendingList that its right branch is not equal to root, then we will process
 *      rightPendingList if its node has right branch.
 *
 * rightPendingList is to store node that its right branch is pending to be processed.
 * topPendingList is to store node that its left branch has processed, right branch is now in process,
 * but its node is not yet processed.
 */
 ...
 *
 * We process left branch of a node, the node and then right branch, we traverse tree in depth first approach,
 * so if there is left branch, we will proceed further left as far as possible, then the node (in bottom) and
 * then right branch, when we are in right branch, we will repeat above.
 *
 * It is simple than postorder logic that we only need a topPendingList that stores a list of node that
 * we want to process after processing the left branch.
 */
 ...
 *
 * It is simple than postorder logic that we only need a rightPendingList that stores a list of right
 * branch node that we want to process after processing the left branch.
 */
